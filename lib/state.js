...
/**
 * Delete a customer using the Customer Number
 * @param {Customer} customer
 * @returns {DeleteCustomerReply}
 */
Elarian.prototype._deleteCustomer = function deleteCustomer(customer) {
    const socket = this.getSocket();
    ...
    cmd = deleteCustomerImpl(cmd, customer);
    const req = new AppToServerCommand()
        .setDeleteCustomer(cmd);
                        ...
                        if (result.data.paymentState && Object.keys(result.data.paymentState).length) {
                            ...
                            result.data.paymentState = {
                                ...
                                transactionLogs: entry.transactionLogList
                                    ...
                                    .map((i) => ({
                                        ...i,
                                        appId: i.appId ? i.appId.value : undefined,
                                        narration: i.narration ? i.narration.value : undefined,
                                        // eslint-disable-next-line max-len
                                        description: i.description ? i.description.value : undefined,
                                    })),
                                pendingTransactions: entry.pendingTransactionsList
                                    // eslint-disable-next-line max-len
                                    .map((i) => ({ ...i, appId: i.appId ? i.appId.value : undefined })),
